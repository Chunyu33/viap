...
### 批量迁移体验与安全增强
- 批量迁移复用 MigrationModal 进度弹窗，通过事件监听推送实时进度，解决批量操作无反馈的问题
- 大文件夹批量迁移新增红色"停止"按钮，迁移中可一键终止（含 useRef 安全取消标志）
- 大文件夹 TARGET_EXISTS 从裸 Toast 改为确认弹窗，支持 force_overwrite 一键覆盖重试
- `migrate_large_folder` 新增 `force_overwrite: Option<bool>` 参数，前端覆盖确认可直达后端
- 批量迁移结束后汇总通知成功/失败数量，失败项列表在 Toast 中展示

### 更新通知修复
- 修复进度条百分比计算错误：从累加字节数改为 `(downloaded / contentLength) * 100` 真实百分比
- 修复 dismiss 取消后后台下载继续运行并触发 relaunch 重启的严重 bug（新增 cancelRef 中断标志）
- 修复样式穿透：移除无 CSS 定义的 `update-banner` 类名，全部改为内联样式 + 外层隔离容器
- 修复 `error` 状态渲染空 div 的问题，加入早期返回过滤
- installing 状态新增关闭按钮，防止 relaunch 失败后用户被锁死
- 更新说明文字超长时自动截断，不撑破布局

### 项目推介弹窗
- 新增 ProjectPromoModal，在设置页底部展示 LightC（C盘清理工具）和 BinlockX（文件加密工具）
- 支持 SVG 图标暗色主题兼容，GitHub / 网盘双下载入口
